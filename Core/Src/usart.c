...
void HAL_UART_MspDeInit(UART_HandleTypeDef* uartHandle)
{
  ...
  /* USER CODE BEGIN USART2_MspDeInit 1 */
	memset(&hdma_usart2_rx,0,sizeof(DMA_HandleTypeDef) );
...
void USART2_DMA_RxCpltCallback(){
	...
	uint32_t temp;
  	...
  	temp = huart2.Instance->SR;  //清除状寄存器SR,读取SR寄存器可以实现清除SR寄存器的功能
  	temp = huart2.Instance->DR; //读取数据寄存器中的数据
  	HAL_UART_DMAStop(&huart2); //
  	temp  = hdma_usart2_rx.Instance->CNDTR;// 获取DMA中未传输的数据个数
  	rx_len =  USART2_RxBUFFER_SIZE - temp; //总计数减去未传输的数据个数，得到已经接收的数据个
...
//	UNUSED(huart);
	...
	if (huart == &huart2){
//		uint16_t buffsize = huart2.RxXferSize;
//		CDC_Transmit_FS(ESP_RxBuffer,buffsize);
//	//	HAL_UART_Transmit(&huart2, (uint8_t*)ESP_RxBuffer, 10, 1000);		/* 接收�????????个字节数据直接发出去 */
//		HAL_UART_Receive_DMA(&huart2,(uint8_t*)ESP_RxBuffer,buffsize); //enable DMA receiving again
	}
